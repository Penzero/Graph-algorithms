...
class TestGraph(unittest.TestCase):
    ...
    def setUp(self):
        self.graph, self.station_to_vertex, self.vertex_to_station = Graph.create_from_metro_file("input.txt")
    ...
    def test_dijkstra(self):
        start_vertex = self.station_to_vertex["s1"]
        distance = self.graph.dijkstra(start_vertex)

        self.assertEqual(distance[start_vertex], 0)
        self.assertTrue(distance[self.station_to_vertex["s2"]] > 0)
